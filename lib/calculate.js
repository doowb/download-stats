...
calculate.group.total = function(groups) {
  ...
  while (len--) {
    ...
    if (Array.isArray(group)) {
      ...
      group.total = calculate.total(group);
    }
  }
  return res;
};
...
calculate.group.avg = function(groups) {
  var res = {};
  var keys = Object.keys(groups);
  var len = keys.length, i = 0;
  while (len--) {
    var key = keys[i++];
    res[key] = calculate.avg(groups[key]);
  }
  return res;
};
...
calculate.group.window = function(days, arr, init) {
  ...
  return calculate.group(arr, function(download) {
    var day = new Date(download.day);
    var diff = daydiff(today, day);
